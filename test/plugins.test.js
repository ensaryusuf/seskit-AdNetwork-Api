...
let { list: ads } = require('../dist/data.json')

describe('Routes', () => {
  it('/ads', async () => {
    const response = await supertest(app).get('/ads')

    expect(response.status).to.equal(200)
    expect(response.body).to.have.property('ads')
  })
  it('/ads/[plugin_name]', async () => {
    const response = await supertest(app).get('/ads/flutter-plugin')

    expect(response.status).to.equal(200)
    expect(response.body).to.have.property('plugin')
  })

  it('/ads/doesnt_exist', async () => {
    const response = await supertest(app).get('/ads/doesnt_exist')
...
describe(`Plugin's releases`, () => {
  ...
  Object.keys(ads).map((n) => {
    const { releases, id, name } = ads[n]
